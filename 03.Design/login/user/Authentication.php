...
class Authentication
{
	...
	function returnIdSchoolForEditAfterCheckingToken() {
		$sql = "SELECT `id_school1` FROM `users` WHERE token = '". $this->TOKEN."' ";
 
        $result = mysqli_query($this->DB_CONNECTION, $sql);
 
        $row = mysqli_fetch_array($result);
            return $row['id_school1'];
	}
	...
	function returnIdImageCheckingToken(){
		$sql = "SELECT `profile_picture` FROM `users` WHERE token = '". $this->TOKEN."' ";
